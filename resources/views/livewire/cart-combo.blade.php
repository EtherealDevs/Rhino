...
<div class="mx-auto w-full" x-data="{ isOpen: false }">
    <a @click.prevent="isOpen = !isOpen" class="flex justify-center items-center">
        <div
            class="mb-3 py-3 mt-6 w-full max-w-screen-xl transform cursor-pointer flex-col justify-between rounded-xl bg-white bg-opacity-75 p-6 text-slate-800 transition shadow-md duration-500 ease-in-out hover:-translate-y-1 hover:shadow-lg lg:flex-row lg:p-4">
            <div class="w-full grid grid-cols-7">

                {{-- Imagenes --}}
                <div class="col-span-2 w-full flex-row lg:w-3/12">
                            ...
                            <div class="relative flex space-x-2">
                                @for ($i = 0; $i < 2; $i++)
                                    <div x-data="{ tooltip: false }"
                                        class="relative inline-flex transition duration-300 ease-in-out" x-cloak>
                                        <img @mouseover="tooltip = true" @mouseleave="tooltip = false"
                                            class="z-10 h-14 w-14 rounded-full border-2 border-white object-cover shadow hover:shadow-xl dark:border-slate-800"
                                            src="{{ url(Storage::url('images/product/' . $items[$i]['item']->images->first()->url)) }}"
                                            alt="Marilyn Monroe" />
                    ...
                    <div x-data="{ tooltip: false }" class="relative inline-flex transition duration-300 ease-in-out" x-cloak>
                        ...
                        <div class="relative z-50 overflow-visible pt-2" x-cloak x-show="tooltip"
                            ...
                            {{-- Precio / Descuento --}}
                            <div class="col-span-1 w-full self-center pt-4 lg:pt-0">
                                <div class="ml-1">
                                    <div
                                        class="text-xl line-through text-gray-500 font-extrabold leading-5 tracking-tight">
                                        ${{ number_format($subtotal, 0, ',', '.') }}</div>
                                    <span
                                        class="text-[12px] ml-2 rounded bg-green-600 px-2 py-1 align-middle font-bold uppercase text-white">
                            ...
                            {{-- Total OFF --}}
                            <div class="col-span-3 grid-cols-2 w-full justify-center pt-4 lg:pt-0">
                                {{-- Title --}}
                                <div class="text-[13px] col-span-1 text-slate-900">Total con Descuento</div>

                                {{-- Precio --}}
                                <div class="ml-1">
                                    <div class="text-xl col-span-1 font-extrabold leading-5 tracking-tight">
                                        <span
                                            class="align-middle text-slate-900">${{ number_format($total, 0, ',', '.') }}</span>
                                    </div>
                                </div>
                            </div>

                            {{-- Eliminar --}}
                            <div class="col-span-1 w-3/4 self-center pt-4 text-black lg:w-1/6 lg:pt-0">
                                <button type="submit"
                        ...
                        <div x-show="isOpen" @click.away="isOpen = false" class="transition p-6 mt-0">
                            @foreach ($items as $item)
                                <div class="flex flex-col sm:flex-row sm:items-center lg:my-8 space-y-0 space-x-4">
                                    ...
                                    <div
                                        class="flex-1 sm:flex sm:justify-between sm:items-center w-full sm:text-center">
